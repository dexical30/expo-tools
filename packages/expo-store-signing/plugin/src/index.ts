...
const withStoreSigning: ConfigPlugin<StoreSigningPluginProps> = (
  config,
  props
) => {
  return withAppBuildGradle(config, (config) => {
    ...
    if (!fs.existsSync(keystorePath)) {
      throw new Error(
        `Keystore file not found at ${keystorePath}. Please check the path and try again.`
      );
    }
...
export function modifyBuildGradle(
  content: string,
  params: AndroidSigningParams
) {
  const envString = `
        release {
